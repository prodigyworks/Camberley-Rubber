...
<?php
	if (isset($_POST['domainurl'])) {
		...
		$checkpointstartdate = convertStringToDate($_POST['checkpointstartdate']);
		...
		$qry = "UPDATE {$_SESSION['DB_PREFIX']}siteconfig SET 
				...
				checkpointstartdate = '$checkpointstartdate',
	...
	$qry = "SELECT *, 
			...
			DATE_FORMAT(checkpointstartdate, '%d/%m/%Y') AS checkpointstartdate 
...
<form id="contentForm" name="contentForm" method="post" class="entryform">
	...
	<label>Start Date (Check Point)</label>
	<input  type="text" class="datepicker" id="checkpointstartdate" name="checkpointstartdate" value="<?php echo $member['checkpointstartdate']; ?>" />
