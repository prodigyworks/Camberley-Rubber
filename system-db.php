		...
		if (! isset($_SESSION['SITE_CONFIG'])) {
			...
			if ($result) {
				if (mysql_num_rows($result) == 1) {
					...
					$data->checkpointstartdate = $member['checkpointstartdate'];
...
function smtpmailer($to, $from, $from_name, $subject, $body, $attachments = array()) { 
	...
	try {
		...
		$mail->SetFrom("office@cbdatasystem.com", $from_name);
		$mail->IsHTML(true);
...
function sendRoleMessage($role, $subject, $message, $attachments = array()) {
	...
	if($result) {
		while (($member = mysql_fetch_assoc($result))) {
			smtpmailer($member['email'], 'office@cbdatasystem.com', 'Camberley Rubber Services', $subject, getEmailHeader() . "<h4>Dear " . $member['firstname'] . ",</h4><p>" . $message . "</p>" . getEmailFooter(), $attachments);
...
function sendInternalRoleMessage($role, $subject, $message, $attachments = array()) {
	$from = "office@cbdatasystem.com";
	$fromName = "Camberley Rubber Services";
...
function sendTeamMessage($id, $subject, $message, $footer = "") {
	...
	if($result) {
		while (($member = mysql_fetch_assoc($result))) {
			smtpmailer($member['email'], 'office@cbdatasystem.com', 'Camberley Rubber Services', $subject, getEmailHeader() . "<h4>Dear " . $member['firstname'] . ",</h4><p>" . $message . "</p>" . getEmailFooter(). $footer);
...
function sendUserMessage($id, $subject, $message, $footer = "", $attachments = array(), $action = "") {
	...
	if($result) {
		while (($member = mysql_fetch_assoc($result))) {
			smtpmailer($member['email'], 'office@cbdatasystem.com', 'Camberley Rubber Services', $subject, getEmailHeader() . "<h4>Dear " . $member['firstname'] . ",</h4><p>" . $message . "</p>" . getEmailFooter(). $footer, $attachments);
