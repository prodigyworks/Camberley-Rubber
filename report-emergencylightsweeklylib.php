<?php
	...
	class LightCheckListReport extends CheckListReport {
		...
		function getTitle() {
			return "Emergency Lights Weekly";
		}
			...
			try {
				...
				if ($result) {
					while (($this->headermember = mysql_fetch_assoc($result))) {
						...
						$this->dynamicY = 44;
						$lightchecklistid = $this->headermember['id'];

			            $sql =  "SELECT
								 A.*
								 FROM  {$_SESSION['DB_PREFIX']}lightschecklistitem A
								 WHERE A.lightchecklistid = $lightchecklistid
								 ORDER BY A.id";
